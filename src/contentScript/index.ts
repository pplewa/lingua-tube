        ...
        textElements.forEach((element, eventIndex) => {
          ...
          if (isAutoGenerated && segments.length > 1) {
            ...
            segments.forEach((segment: { utf8: string; tOffsetMs?: number }, segIndex: number) => {
              ...
              // Build cumulative native text up to this point
              const cumulativeNativeText = nativeSegments
                .slice(0, segIndex + 1)
                .map((seg: { utf8: string; tOffsetMs?: number }) => seg.utf8 || '')
                .join('');

              const segmentStartTime = start + (segment.tOffsetMs || 0);
              const nextSegment = segments[segIndex + 1];
              const segmentDuration = nextSegment 
                ? (nextSegment.tOffsetMs || 0) - (segment.tOffsetMs || 0)
                : dur - (segment.tOffsetMs || 0);
              ...
              cues.push({
                ...
                text: cumulativeText,
                nativeText: cumulativeNativeText,
