...
export class DictionaryApiService implements IDictionaryService {
   ...
   */
  private async fetchDefinitionFromApi(word: string, language: LanguageCode): Promise<WordDefinition> {
    ...
    for (let attempt = 1; attempt <= this.config.retryAttempts; attempt++) {
      try {
        ...
        if (error instanceof Error && error.name === 'AbortError') {
          lastError = this.createDictionaryError(
            'Request timeout',
            DictionaryErrorCode.TIMEOUT,
            { word, language, attempt }
          );
        }
    ...
    // All retries failed
    throw lastError || this.createDictionaryError(
