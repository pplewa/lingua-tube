...
export class ErrorHandlingService {
  ...
  private circuitBreakerMetrics: Map<string, {
    failures: number;
    lastFailure: number;
    lastTest: number;
  }> = new Map();
  ...
  private async updateHealthStatus(service: string): Promise<void> {
    ...
    const recentErrors = this.errorHistory
      .filter(e => e.service === service && e.timestamp > fiveMinutesAgo);
  ...
  async getSystemHealth(): Promise<SystemHealth> {
    ...
    const activeErrors = this.errorHistory
      .filter(e => e.timestamp > tenMinutesAgo)
      .slice(0, 50); // Limit to recent 50 errors
  ...
  private generateRecommendations(
    ...
    Object.values(services).forEach(service => {
      if (service.circuitBreakerState === CircuitBreakerState.OPEN) {
        recommendations.push(`Circuit breaker is open for ${service.service}. Wait for automatic recovery or investigate the underlying issue.`);
      }
    });

    // Check for high error rates
    Object.values(services).forEach(service => {
      if (service.errorRate > 0.1) { // More than 0.1 errors per second
        recommendations.push(`High error rate detected for ${service.service}. Check service configuration and logs.`);
      }
    });

    // Check for quota issues
    const quotaErrors = activeErrors.filter(e => e.errorCode === TranslationErrorCode.QUOTA_EXCEEDED);
    if (quotaErrors.length > 0) {
      recommendations.push('Translation quota exceeded. Consider upgrading your plan or optimizing translation usage.');
    }

    // Check for rate limiting
    const rateLimitErrors = activeErrors.filter(e => e.errorCode === TranslationErrorCode.RATE_LIMIT_EXCEEDED);
    if (rateLimitErrors.length > 5) {
      recommendations.push('Frequent rate limiting detected. Consider implementing request throttling or upgrading your plan.');
    }
