...
export interface SubtitleTrack {
  ...
  readonly groups?: Array<{ startTime: number; endTime: number; text: string }>;
}
...
export class PlayerInteractionService {
  ...
  public parseYouTubeSubtitleData(
    ...
    for (const event of data.events) {
      ...
      const text = segments.map((seg: any) => seg?.utf8 || '').join('').replace(/\n+/g, '\n').trim();

      if (!text || !isFinite(startTime) || !isFinite(endTime) || endTime <= startTime) continue;
      ...
      cues.push({
        ...
        segments: segments.map((s: any) => ({ utf8: s?.utf8 || '', tOffsetMs: s?.tOffsetMs }))
      });
      if (isAutoGenerated) {
        groups.push({ startTime, endTime, text });
      }
    }
    ...
    return {
      ...
      cues,
      source: 'youtube',
      groups: isAutoGenerated ? groups : undefined,
    };
  }
