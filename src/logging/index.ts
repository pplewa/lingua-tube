...
export const log = {
  ...
  endOperation: (name: string, additionalMetadata?: Partial<import('./PerformanceMonitor').OperationMetadata>) => 
    logger.endPerformanceOperation(name, additionalMetadata),
  measureAsync: <T>(name: string, operation: () => Promise<T>, metadata: import('./PerformanceMonitor').OperationMetadata) => 
    logger.measureAsyncOperation(name, operation, metadata),
  measureSync: <T>(name: string, operation: () => T, metadata: import('./PerformanceMonitor').OperationMetadata) => 
...
export const createComponentLogger = (component: ComponentType) => ({
  ...
  startOperation: (name: string, operationType: string, metadata?: Partial<import('./PerformanceMonitor').OperationMetadata>) => 
    logger.startPerformanceOperation(name, { component, operationType, ...metadata }),
  endOperation: (name: string, additionalMetadata?: Partial<import('./PerformanceMonitor').OperationMetadata>) => 
    logger.endPerformanceOperation(name, additionalMetadata),
  measureAsync: <T>(name: string, operation: () => Promise<T>, operationType: string, metadata?: Partial<import('./PerformanceMonitor').OperationMetadata>) => 
    logger.measureAsyncOperation(name, operation, { component, operationType, ...metadata }),
  measureSync: <T>(name: string, operation: () => T, operationType: string, metadata?: Partial<import('./PerformanceMonitor').OperationMetadata>) => 
