...
export interface RateLimitConfig {
  ...
  readonly componentLimits: Partial<Record<ComponentType, {
    readonly maxLogsPerSecond: number
    readonly burstLimit: number
  }>>
...
export interface RateLimitStats {
  ...
  readonly byComponent: Record<ComponentType, {
    readonly total: number
    readonly blocked: number
    readonly rate: number
  }>
  readonly byLevel: Record<LogLevel, {
    readonly total: number
    readonly blocked: number
    readonly rate: number
  }>
...
export class RateLimitingService {
  ...
  private rateLimitStats: Map<string, { total: number; blocked: number; lastProcessingTime: number }> = new Map()
  
  // Deduplication state
  private deduplicationCache: Map<string, DeduplicationEntry> = new Map()
  private deduplicationStats: Map<string, { total: number; unique: number; lastProcessingTime: number }> = new Map()
   ...
   */
  public checkDeduplication(entry: LogEntry): { shouldLog: boolean; dedupInfo?: DeduplicationEntry } {
    if (!this.deduplicationConfig.enabled) {
   ...
   */
  private updateDeduplicationStats(component: ComponentType, shouldLog: boolean, processingTime: number): void {
    const key = component
  ...
  private cleanupRateLimitData(): void {
    ...
    for (const [key, bucket] of this.tokenBuckets.entries()) {
      if (now - bucket.lastRefill > windowMs * 10) { // 10x window for cleanup
  ...
  public getRateLimitStats(): RateLimitStats {
    const totalRequests = Array.from(this.rateLimitStats.values())
      .reduce((sum, stats) => sum + stats.total, 0)
    const blockedRequests = Array.from(this.rateLimitStats.values())
      .reduce((sum, stats) => sum + stats.blocked, 0)
    
    const byComponent: Record<ComponentType, { total: number; blocked: number; rate: number }> = {} as any
    ...
    const avgProcessingTime = Array.from(this.rateLimitStats.values())
      .reduce((sum, stats) => sum + stats.lastProcessingTime, 0) / this.rateLimitStats.size || 0
  ...
  public getDeduplicationStats(): DeduplicationStats {
    const totalEntries = Array.from(this.deduplicationStats.values())
      .reduce((sum, stats) => sum + stats.total, 0)
    const uniqueEntries = Array.from(this.deduplicationStats.values())
      .reduce((sum, stats) => sum + stats.unique, 0)
    ...
    const byComponent: Record<ComponentType, { total: number; unique: number; duplicated: number }> = {} as any
    ...
    const avgProcessingTime = Array.from(this.deduplicationStats.values())
      .reduce((sum, stats) => sum + stats.lastProcessingTime, 0) / this.deduplicationStats.size || 0
    
    return {
      totalEntries,
      uniqueEntries,
      duplicatedEntries: totalEntries - uniqueEntries,
      deduplicationRate: totalEntries > 0 ? ((totalEntries - uniqueEntries) / totalEntries) * 100 : 0,
      topDuplicates,
      byComponent,
